...
#from tensorflow.python.keras.layers import Dense, Activation, Embedding, Input
...
### my code
def create_model(ortho_reg1, neg_size, emb_dim, aspect_size, emb_path, maxlen, vocab):

    def ortho_reg(weight_matrix):
        ### orthogonal regularization for aspect embedding matrix ###
        w_n = weight_matrix / K.cast(K.epsilon() + K.sqrt(K.sum(K.square(weight_matrix), axis=-1, keepdims=True)), K.floatx())
        reg = K.sum(K.square(K.dot(w_n, K.transpose(w_n)) - K.eye(w_n.shape[0].eval())))
        return ortho_reg1*reg
